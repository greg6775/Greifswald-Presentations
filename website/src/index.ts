    ...
    Wizard({
        ...
        Page({
            ...
            Grid(
                ...
                DropDownInput("Vortrag 1 (14:00 - 14:20)", presentations[ 0 ].presentations.map(x => x.id)).setRender((key) => `${key[ 3 ] ? `${key[ 3 ]}:` : ""} ${topics.find(z => z.id == key.substring(0, 3))!.title}`).sync(data, "p1"),
                DropDownInput("Vortrag 2 (14:30 - 15:00)", presentations[ 1 ].presentations.map(x => x.id)).setRender((key) => `${key[ 3 ] ? `${key[ 3 ]}:` : ""} ${topics.find(z => z.id == key.substring(0, 3))!.title}`).sync(data, "p2"),
                DropDownInput("Vortrag 3 (15:10 - 15:20)", presentations[ 2 ].presentations.map(x => x.id)).setRender((key) => `${key[ 3 ] ? `${key[ 3 ]}:` : ""} ${topics.find(z => z.id == key.substring(0, 3))!.title}`).sync(data, "p3"),
                DropDownInput("Vortrag 4 (15:30 - 15:50)", presentations[ 3 ].presentations.map(x => x.id)).setRender((key) => `${key[ 3 ] ? `${key[ 3 ]}:` : ""} ${topics.find(z => z.id == key.substring(0, 3))!.title}`).sync(data, "p4"),
